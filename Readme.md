
# 🌐 Mini-Cache TTL — Демонстрация распределённого кэширования

Этот проект демонстрирует работу пакета `mini-cache-ttl`( [`npm`](https://www.npmjs.com/package/mini-cache-ttl) ) | ([`git`](https://github.com/O9nix/mini-cache-ttl/tree/feature/server-client) )в режиме:

- **Локального in-memory кэша** (изолированный для каждого сервиса)
- **Централизованного сетевого кэша** (общий для всех сервисов)

Идеально подходит для понимания гибридной архитектуры кэширования:  
> «Быстро локально + согласованно глобально».

---

## 🧩 Архитектура
```
mini-cache-demo
├─ cache-server
│  ├─ package-lock.json
│  ├─ package.json
│  └─ server.js
├─ Readme.md
├─ server-a
│  ├─ app.js
│  ├─ package-lock.json
│  ├─ package.json
│  └─ public
│     └─ index.html
├─ server-b
│  ├─ app.js
│  ├─ package-lock.json
│  ├─ package.json
│  └─ public
│     └─ index.html
└─ test-cache
   ├─ package-lock.json
   ├─ package.json
   ├─ test-local.mjs
   └─ test-remote.mjs
```
- **Общий кэш**: `http://localhost:4000` → хранилище `shared-data`
- **Сервис A**: `http://localhost:3001`
- **Сервис B**: `http://localhost:3002`

---

## 🚀 Быстрый запуск

1. Установка зависимости во всех папках:
   ```bash
   # В корне проекта
   npm install --prefix cache-server
   npm install --prefix server-a
   npm install --prefix server-b
   npm install --prefix test-cache
   ```

2. Запустк серверов в трёх отдельных терминалах:

```bash
# Терминал 1: общий кэш
npm start --prefix cache-server

#либо отдельно через CLI
npx mini-cache-ttl-server
# На порту 4000
npx mini-cache-ttl-server --port 4000
# С токеном
npx mini-cache-ttl-server -p 4000 -t my-secret-key

# Терминал 2: сервис A
node server-a/app.js

# Терминал 3: сервис B
node server-b/app.js
```

3. Открой в браузере:
http://localhost:3001 → Сервис A
http://localhost:3002 → Сервис B

4. Что можно сделать

## В веб-интерфейсе:
✅ Добавлять/удалять записи в локальном кэше (видны только в этом сервисе)

✅ Добавлять/удалять записи в общем кэше (видны в обоих сервисах)

✅ Просматривать список ключей

✅ Управлять TTL (временем жизни)

## Через API (примеры):

```bash
# Запись в общий кэш из терминала
curl -X POST http://localhost:3001/shared/message \
  -H "Content-Type: application/json" \
  -d '{"value":"Привет из CLI!", "ttl":60}'

# Чтение из другого сервиса
curl http://localhost:3002/shared/message
```


# Проверка полного API

Пакет ```mini-cache-ttl``` поддерживает все методы:
get, set, update, del, rename
keys, size, clear
touch, has
Проверить их можно через скрипты:

```bash
# Локальный кэш
node test-cache/test-local.mjs

# Удалённый кэш (требуется запущенный cache-server)
node test-cache/test-remote.mjs
```

# Дашборд и стресс тест

### Дашборд
Запустить сервис с дашбордом можно двумя способами

через код
```javascript
import { createApp } from 'mini-cache-ttl/server';
import { monitor } from 'mini-cache-ttl/monitor';

const { app, stores } = createApp();

// Подключаем мониторинг
monitor(app, { stores });

app.listen(4000, () => {
  console.log('✅ Кэш-сервер с мониторингом запущен на http://localhost:4000');
});
```

через консоль

```bash
npx mini-cache-ttl-server -p 4000 --monitor
# или
npx mini-cache-ttl-server -p 4000 -m

```
Для просмотра нужно перейти по адресу
`http://localhost:PORT/dashboard`

### Стресс тест

Для запуска стресс теста введите команду в терминале

```bash
cd ./test-cache
node stress-test.mjs
```

вы можете изменить свойства теста либо в переммных в коде либо передать их при запуске

CLI
```bash
node stress-test.mjs --query 10000 --parquery 300 --ttl 120
#или
node stress-test.mjs -q 1000 -l 300 -t 1

# --query (-q) общее количество запросов
# --parquery (-l) количество параллельных запросов
# --ttl (-t) Время жизни одной записи в кэше 
# если поставить r то время будет случайным
```